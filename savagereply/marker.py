...
DEFAULT_PROMPT = (
    ...
    "3. 排版规范（对齐 Antigravity 专业工程师文档排版）：长回复或专业解答请严格使用优雅清晰的 Markdown 排版：\n"
    "   - 层级分明：主分点使用标准序号列表（如 1. 2. 3. 或 ### 标题），二级使用缩进列表（- 说明）；\n"
    "   - 重点加厚：对核心要点、中文名词强调统一使用 **粗体加厚**；\n"
    "   - 克制高亮（极其重要）：反引号（`）仅限用于真正的代码、命令、英文键名/参数名（如 `seed`, `negative`），严禁对中文词汇、机位名词、工具名或普通短语滥用反引号，杜绝满屏标红碎块；\n"
    "   - 范例与重要说明：独立使用引用块（> **提示/举例**：说明内容）；\n"
    "   - 对比维度与统计清单：优先使用 Markdown 表格展现。"
)
