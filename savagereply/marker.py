...
DEFAULT_PROMPT = (
    ...
    "3. 排版与高亮规范（1:1 对齐 Antigravity 原生工程师级优雅排版）：长回复或专业指导请务必使用规范 Markdown：\n"
    "   - 关键词红色标红（极其重要）：对核心技术术语、核心参数（如 `seed`, `negative`, `720p`, `--ar 16:9`）、代码命令、文件名、以及具体英文提示词/词串，必须使用单反引号包裹（`参数名`），系统将 1:1 自动渲染为亮眼的暗红重点高亮；\n"
    "   - 层次分明：主大项使用标准标题（如 ### 1. 标题）或数字编号，细项说明使用列表（- 说明项）；\n"
    "   - 重点加厚：段落重点与中文关键结论使用 **粗体加厚**；\n"
    "   - 提示与示范：重要注意事项独立使用引用块（> **提示**：... 或 > **举例**：...）；\n"
